...
impl CNN<u16, u16>
// trying to make it generic would make it so I had to use hashmaps
// So I'm just doing the specific implementation
{
  pub fn with_config(
    embedding_dim: usize,
    num_filters: usize,
  ...
  fn build_vocabulary(&mut self, vocab_size: usize, label_count: usize) {
  ...
  fn embed_sequence(&self, token_ids: &[u16]) -> Vec<Vec<f32>> {
    token_ids
      .iter()
      .map(|&token_id| self.embedding_weights[token_id as usize].clone())
      .collect()
  }
  ...
  fn forward_pass(&self, sequence: &[u16]) -> Vec<f32> {
    // let token_ids = self.tokenize_sequence(sequence);
    let token_ids = sequence;
  ...
  fn encode_labels(&self, labels: &[u16]) -> Vec<f32> {
...
impl MachineLearningModel<u16, u16> for CNN<u16, u16> {
  fn new() -> Self {
    CNN::with_config(
      128,           // embedding_dim
      100,           // num_filters
  ...
  fn predict(&self, data_point: &Vec<u16>) -> Vec<u16> {
    ...
    class_probs
      ...
      .filter_map(|(class_id, _)| Some(*class_id as u16))
      .collect()
  }
}
...
mod tests {
  ...
  fn test_training_and_prediction() {
    ...
    // Sample tokenized text data (token IDs directly)
    let data = vec![
      vec![1u16, 2, 3, 4, 5],   // Token IDs for document 1
      vec![2u16, 3, 4, 6, 7],   // Token IDs for document 2
      vec![1u16, 3, 5, 8, 9],   // Token IDs for document 3
      vec![4u16, 6, 7, 10, 11], // Token IDs for document 4
    ];

    // Sample labels (class IDs for each document)
    let labels = vec![
      vec![0u16], // Class ID 0
      vec![1u16], // Class ID 1
      vec![2u16], // Class ID 2
    ...
    // the result with the current test is 0,2,1
    // which is good because 0 has all 3 words, 2 has two and 1 has two, idk about the probabilities of 1 and 2, they should be the same though
  }
}
